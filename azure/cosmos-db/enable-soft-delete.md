---
title: Soft delete for Azure Cosmos DB (preview)
...
Azure Cosmos DB soft delete is a data resiliency feature that retains deleted resources (accounts, databases, containers) for a configurable period (default 14 days) instead of permanently removing them at once. Soft delete enables fast recovery from accidental deletions without requiring a restore from backups. In effect, when soft delete is active, deleting a Cosmos DB resource will marks it as “soft-deleted”. The resource remains internally preserved and can be restored within the retention window. After the retention period expires or if an authorized user explicitly purges it earlier, the resource is permanently deleted.  

Soft delete feature addresses a critical operational need: accidental deletion of databases or containers can lead to extended downtime and complex restore  procedures. Soft delete reduces downtime by allowing in-place recovery of deletions in minutes. This feature provides a safety net so that if a Cosmos DB resource is mistakenly removed, it can be quickly recovered with minimal disruption. 

## Key benefits and use cases 

- **Rapid recovery and minimal downtime**: Soft delete allows a deleted Cosmos DB resource to be restored within minutes, since the data never truly left the service. 
...
- **Protection against human error**: It acts as a safety net for mistakes. If an administrator or automation script erroneously deletes a database or container, the data is not lost – it can be undeleted promptly. This significantly lowers the risk of catastrophic data loss due to user error and avoids business downtime. 

- **Simplified restore process**: Recovery via soft delete is simpler than a backup-based restore. The resource is brought back in-place with the same name, 
...
The feature is enabled at the Cosmos DB account level along with the retention policy.   Once soft delete is turned on for an account, all deletions of databases or containers in that account use soft-delete behavior. You can't target specific databases or containers to have different settings – because it's an account-wide setting. If soft delete isn't enabled on an account, deletions operate normally with immediate permanent deletion.  
...
soft delete enabled account soft-deletes that database and all containers within it. Deleting an individual Container (collection/graph/table) soft-deletes just that container. This cascading behavior ensures no orphaned resources remain and that a restore of a parent (like an account or database) can bring back all its children except in cases where child resources were soft deleted. 

### Soft-deleted state

Once a resource (account, database, or container) is soft-deleted, it enters a retained but inaccessible state:  

- For example, a soft-deleted container doesn't appear when listing containers in its database. 

- Any attempt to access the data behaves as if the resource doesn’t exist. Applications receive 404 Not Found errors for queries or updates on a soft-deleted container, identical to the behaviour of permanently deleted one. This behaviour ensures the soft-deleted data isn't inadvertently used or modified while pending deletion.  
 
- Internally, Cosmos DB retains all the data and metadata for the resource. The deletion is logical. The underlying data files and configurations are preserved for the duration of the retention period. The soft-deleted resource is hidden from the user’s perspective but still present in the system. 

## Minimum minutes before permanent delete 

**Retention period**: Soft-deleted resources remain recoverable for a configured retention Period. By default, the retention period is 14 days (plans to be reduced to 1 day in public preview), but administrators can adjust this setting per account. You might choose a shorter retention (minimum 3 day) for lower storage overhead or a longer period (e.g., 30 days) for extra safety. During this retention window, the resource can be restored at any time. Once the 
...
For example, Azure CLI commands would be available for recovery  of database or container. The Azure Portal will also offer a user-friendly interface (e.g., a “Recycle Bin” or recover options in Data Explorer) to select a soft-deleted item and recover it. All recover operations are protected by Azure RBAC (e.g., only Cosmos DB Account Contributors or higher roles can initiate an undelete).  

## Permanent deletion (purge)

If you do not recover a soft-deleted resource within the retention period, Cosmos DB will automatically purge it after the time elapses. However, there may be cases where you want to permanently delete a soft-deleted resource before the retention window ends – for example, if a test resource was deleted and you’re sure you won’t need it, or for compliance reasons you need it gone immediately. The soft delete feature supports manual purge of a soft-deleted resource: 
...
Only users with sufficient privileges (for example, Cosmos DB Account Contributor or Owner roles) can soft-delete or restore resources. Soft-deleted data is not accessible to any read or write operations, so it remains secure in that interim state.  

### Management interfaces

Soft delete can be managed through all standard Azure interfaces:

- **Azure portal** provides UI to enable/disable the feature at the account level and to enumerate and restore soft-deleted resources (e.g., a “Deleted Items” 
list with the option to recover).  Not Available in Private Preview. 
...
- **Azure Resource Manager (REST API)**  New properties on the Cosmos DB account resource for soft delete settings (like retention duration) and new 
APIs to list and restore deleted resources are being introduced.  


## Getting Started

Ready to get started? First, register your subscription for access to the preview. Then, check out our quick start guide to configure soft delete.

## Register your subscription

Register your subscription for Cosmos DB soft delete feature. Here are the steps to request access. Please note that registration is not automatic and can take up to one week to process.

1. Go to the Preview Features page in your Azure subscription.

2. Filter by soft delete preview in the feature name search.

3. Select the Register button.

:::image type="content" source="media/soft-delete/cosmosdbsoftdeleteregister.png" alt-text="Screenshot showing the Azure Cosmos DB soft delete preview feature registration in the Azure portal.":::

## Frequently asked questions

### What is soft delete in Azure Cosmos DB?

Soft delete is a feature that retains deleted Cosmos DB resources (accounts, databases, containers) for a configurable retention period (default 14 days (plans to change it to 1 day), allowing recovery before permanent deletion.

### How do I enable soft delete?

Soft delete is enabled at the Cosmos DB account level. Once enabled, all deletions within the account follow soft-delete behavior. In private preview, enablement is managed by the Cosmos DB product team.

### Can I recover a deleted container or database?

Yes. During the retention period, authorized users can restore soft-deleted resources to their original state. In private preview, recovery must be requested through the Cosmos DB team.
...
Yes. The retention period is configurable per account, from 1 to 30 days. In private preview, this setting is fixed and managed by the Cosmos DB team.
