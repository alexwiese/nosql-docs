...
| OpenTcpConnectionTimeout      | 5 seconds     | 1 second  | This value sets the amount of time allowed for trying to establish a connection. Sub-second values in the range [0, 1s) are normalized to 0, causing the transport layer to use `RequestTimeout` as the open-connection timeout instead. Values greater than or equal to 1 second are rounded up to the nearest whole second (for example, 2.3s becomes 3s), clamped to `int.MaxValue` seconds. Negative values are preserved for backward compatibility but emit a warning trace and effectively fall back to `RequestTimeout`. |
