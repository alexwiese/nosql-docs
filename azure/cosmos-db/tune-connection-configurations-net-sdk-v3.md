---
title: Connection Configurations for .NET SDK V3
...
| MaxTcpConnectionsPerEndpoint        | 65535           | 65535        | This value sets the maximum number of TCP connections that the client can open to each Azure Cosmos DB back end. Together with `MaxRequestsPerTcpConnection`, this setting limits the number of requests that are simultaneously sent to a single Azure Cosmos DB back end (`MaxRequestsPerTcpConnection` x `MaxTcpConnectionsPerEndpoint`). Any positive value is accepted. Values greater than or equal to 16 are recommended for optimal performance, but lower positive values are permitted. |
| OpenTcpConnectionTimeout      | 5 seconds     | 1 second  | This value sets the amount of time allowed for trying to establish a connection. Subsecond values in the range [0, 1 s) are normalized to 0, causing the transport layer to use `RequestTimeout` as the open-connection timeout instead. Values greater than or equal to 1 second are rounded up to the nearest whole second (for example, 2.3 s becomes 3 s), clamped to `int.MaxValue` seconds. Negative values are preserved for backward compatibility but emit a warning trace and effectively fall back to `RequestTimeout`. |
